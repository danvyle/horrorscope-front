...
render() {
  return (
    ...
    <p> {(this.props.retrogrades[5] && this.state.isPlanetClicked && this.state.isRetrograde) ? <div className = 'background-modal'> <div className = "modal"> {this.props.retrogrades[5].explanation} <span className="close-button"> <button onClick={this.setDisplay}  className="closed">X</button> </span> </div> </div> : null } </p>
    </div>
  );
}
}
